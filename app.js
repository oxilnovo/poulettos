(() => {
  ...
  function normalizeState(s){
    ...
    base.hens=(s?.hens||[]).map(h=>({...h,status:h.status||'active',deceasedAt:h.deceasedAt||null,updatedAt:h.updatedAt||null,createdBy:h.createdBy||'',editedBy:h.editedBy||'',deleted:!!h.deleted})).filter(h=>!h.deleted);
    base.entries=(s?.entries||[]).flatMap(e=>{
      const hid=e.henId ?? (e.henIds?.[0]||UNKNOWN);
      return [{id:e.id||uid(),date:e.date,weight:Number(e.weight)||0,henId:hid,note:e.note||'',updatedAt:e.updatedAt||null,createdBy:e.createdBy||'',createdAt:e.createdAt||e.updatedAt||null,editedBy:e.editedBy||'' ,deleted:!!e.deleted}];
    }).filter(e=>!e.deleted);
  ...
  $('#entryForm').addEventListener('submit',e=>{e.preventDefault();const date=$('#entryDate').value,rows=$$('.egg-row').map(r=>({weight:Number(r.querySelector('.egg-weight').value),henId:r.querySelector('.egg-hen').value}));if(!date||rows.some(x=>!Number.isFinite(x.weight)||x.weight<=0)){toast('Renseignez le poids de chaque œuf');return}const stamp=`${date}T12:00:00`;
    if(editingId){const e0=state.entries.find(x=>x.id===editingId);if(e0){const now=new Date().toISOString();Object.assign(e0,{date:stamp,weight:rows[0].weight,henId:rows[0].henId,note:$('#note').value.trim(),updatedAt:now,editedBy:state.user.email||state.user.name||'Utilisateur'});save();toast('Œuf modifié ✓')}editingId=null;navigate('history');syncSoon();return}
    rows.forEach(x=>{const now=new Date().toISOString();state.entries.unshift({id:uid(),date:stamp,weight:x.weight,henId:x.henId,note:$('#note').value.trim(),updatedAt:now,createdAt:now,createdBy:state.user.email||state.user.name||'Utilisateur',editedBy:''})});state.entries.sort((a,b)=>entryDayKey(b.date).localeCompare(entryDayKey(a.date)));save();toast(`${rows.length} œuf${rows.length>1?'s':''} enregistré${rows.length>1?'s':''} ✓`);navigate('home');syncSoon();
  });

  function renderHome(){const today=entryDayKey(new Date()),todayE=state.entries.filter(e=>entryDayKey(e.date)===today),week=entriesFor('week'),month=entriesFor('month');$('#todayCount').textContent=total(todayE);$('#weekCount').textContent=total(week);$('#monthCount').textContent=total(month);$('#totalCount').textContent=total(state.entries);$('#helloName').textContent=(state.user.name||'vous').split(' ')[0]||'vous';const recent=[...state.entries].sort((a,b)=>{const d=entryDayKey(b.date).localeCompare(entryDayKey(a.date));return d||String(b.updatedAt||'').localeCompare(String(a.updatedAt||''))}).slice(0,6);$('#recentList').innerHTML=recent.map(e=>`<button class="list-row clickable" data-edit="${e.id}"><div class="row-left"><span class="egg-mini">🥚</span><div><strong>${Number(e.weight).toFixed(0)} g</strong><div class="row-time">${fmt(dateOnly(entryDayKey(e.date)))} · ${escapeHtml(henName(e.henId))}</div></div></div><span class="chev">›</span></button>`).join('')||'<div class="list-row"><span class="row-time">Aucune saisie pour le moment.</span></div>';$$('[data-edit]').forEach(b=>b.onclick=()=>editEntry(b.dataset.edit))}
  ...
  function closeHenModal(){$('#henModal').classList.add('hidden')}$('#addHen').onclick=()=>openHenModal('');$('#closeHenModal').onclick=closeHenModal;$('#henModal').addEventListener('click',e=>{if(e.target.id==='henModal')closeHenModal()});$('#archiveHenBtn').onclick=()=>{const id=$('#henForm').dataset.id,h=state.hens.find(x=>x.id===id);if(!h)return;if(h.status==='dead'){h.status='active';h.deceasedAt=null;h.updatedAt=new Date().toISOString();h.editedBy=state.user.email||state.user.name||'Utilisateur';toast('Poule réactivée')}else{if(!confirm(`Archiver ${h.name} comme décédée ? Ses anciennes données seront conservées.`))return;h.status='dead';h.deceasedAt=$('#deceasedDate').value||isoDate(new Date());h.updatedAt=new Date().toISOString();h.editedBy=state.user.email||state.user.name||'Utilisateur';toast('Poule archivée')}save();closeHenModal();renderHens();renderStats();syncSoon()};$('#henImage').addEventListener('change',()=>{const file=$('#henImage').files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{const max=500,scale=Math.min(1,max/img.width,max/img.height),c=document.createElement('canvas');c.width=Math.round(img.width*scale);c.height=Math.round(img.height*scale);c.getContext('2d').drawImage(img,0,0,c.width,c.height);$('#henPreview').innerHTML=`<img src="${c.toDataURL('image/jpeg',.82)}" alt="">`;$('#henPreview').dataset.photo=c.toDataURL('image/jpeg',.82)};img.src=reader.result};reader.readAsDataURL(file)});$('#henForm').addEventListener('submit',e=>{e.preventDefault();const id=e.currentTarget.dataset.id,name=$('#henName').value.trim();if(!name){toast('Renseignez un nom');return}let h=state.hens.find(x=>x.id===id);if(!h){h={id:uid(),name:'',breed:'',emoji:'🐔',status:'active',deceasedAt:null,createdBy:state.user.email||state.user.name||'Utilisateur',createdAt:new Date().toISOString()};state.hens.push(h)}h.name=name;h.breed=$('#henBreed').value.trim();h.emoji=$('#henEmoji').value.trim()||'🐔';if($('#henPreview').dataset.photo)h.photo=$('#henPreview').dataset.photo;if(h.status==='dead')h.deceasedAt=$('#deceasedDate').value||h.deceasedAt||isoDate(new Date());h.updatedAt=new Date().toISOString();h.editedBy=id?(state.user.email||state.user.name||'Utilisateur'):'';save();closeHenModal();renderHens();renderStats();toast(id?'Poule modifiée ✓':'Poule ajoutée ✓');syncSoon()});

  function renderHistory(){
    const sortSel=$('#historySort'),orderSel=$('#historyOrder'),search=$('#historySearch');
    if(sortSel)sortSel.value=historySort;if(orderSel)orderSel.value=historyOrder;
    const q=String(search?.value||'').trim().toLocaleLowerCase('fr');
    const all=[...state.entries];
    const filtered=q?all.filter(e=>[entryDayKey(e.date),henName(e.henId),String(e.weight||''),e.note||'',e.id||'',e.createdBy||'',e.editedBy||''].join(' ').toLocaleLowerCase('fr').includes(q)):all;
    $('#historyCount').textContent=`${filtered.length} œuf${filtered.length>1?'s':''}${q?' sur '+all.length:''}`;
    const rows=filtered.sort((a,b)=>{
      ...
      else if(historySort==='note'){av=(a.note||'').toLocaleLowerCase('fr');bv=(b.note||'').toLocaleLowerCase('fr');}
      else if(historySort==='createdBy'){av=(a.createdBy||'').toLocaleLowerCase('fr');bv=(b.createdBy||'').toLocaleLowerCase('fr');}
      else if(historySort==='editedBy'){av=(a.editedBy||'').toLocaleLowerCase('fr');bv=(b.editedBy||'').toLocaleLowerCase('fr');}
      else if(historySort==='id'){av=String(a.id||'').toLocaleLowerCase('fr');bv=String(b.id||'').toLocaleLowerCase('fr');}
    ...
    $('#historyList').innerHTML=rows.map(e=>`<div class="history-row"><button class="history-main" data-edit="${e.id}"><div class="history-egg">🥚</div><div><strong>${Number(e.weight).toFixed(0)} g · ${escapeHtml(henName(e.henId))}</strong><div class="row-time">${fmt(dateOnly(entryDayKey(e.date)))}${e.note?' · '+escapeHtml(e.note):''}${e.editedBy?' · modifié':''}</div></div></button><button class="delete-btn" data-delete="${e.id}" aria-label="Supprimer">🗑️</button></div>`).join('')||'<div class="history-row"><span class="row-time">Aucun résultat.</span></div>';
  ...
  $('#historyOrder')?.addEventListener('change',e=>{historyOrder=e.target.value;renderHistory()});$('#historySearch')?.addEventListener('input',renderHistory);
